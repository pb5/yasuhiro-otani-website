...
- `lesson-site/` — **not part of the GitHub Pages site** (excluded via `robots.txt`). A self-contained landing page (own `css/`, `js/`, `images/`) for the lessons business, meant to be uploaded to the Lolipop rental server as `lesson.yasuhiro-otani.com`, decoupled from the artist site so the lessons business has room for richer audio/video and persuasive, ad-facing design without being tied to the main site's minimal editorial tone. Content is fact-checked against `profile.html`/`artistic.html` — no fabricated testimonials or claims.
